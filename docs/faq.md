...
Not by default — PicoCluster Claw is designed for home LAN use with a strict firewall. For remote access, install [Tailscale](https://tailscale.com) on clusterclaw and your remote devices:
...
Yes. See the **Adding Your Own MCP Server** section of [mcp-tools.md](mcp-tools.md) — there's a ~20-line Python template. You drop a new file in `/opt/clusterclaw/mcp/`, add one line to `threadweaver/start.sh` to auto-connect it, and rebuild the ThreadWeaver container:
