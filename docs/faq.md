# PicoCluster Claw FAQ

Frequently asked questions about running your PicoCluster Claw cluster. 26 questions across getting started, models, tools, operations, and privacy. If you don't find your answer here, check [access-guide.md](access-guide.md) for connection details, [mcp-tools.md](mcp-tools.md) for tool reference, or [examples.md](examples.md) for things to try.
...
The control panel lives on picocluster-claw itself and is reachable over plain HTTP on your LAN:

```
http://picocluster-claw/
```
...
```bash
ssh -L 5174:localhost:5174 -L 18790:localhost:18790 picocluster@picocluster-claw
```
...
You'll see a "self-signed certificate" warning the first time — accept it. (The cert is generated by Caddy running on picocluster-claw, and the tunnel is already encrypted by SSH, so the warning is cosmetic.)

Pro tip: add this to `~/.ssh/config` on your computer so you can just type `ssh picocluster-claw`:
...
For security. ThreadWeaver and OpenClaw are only bound to the picocluster-claw loopback interface (`127.0.0.1`), so nothing on your local network can reach them directly without authenticating via SSH first.

The tunnel forwards two local ports to picocluster-claw's loopback:
...
- Caddy on picocluster-claw uses a locally-issued certificate for `localhost`, not one signed by a public CA (Let's Encrypt etc.). Public CAs won't issue certificates for `localhost`.
...
Not by default — PicoCluster Claw is designed for home LAN use with a strict firewall. For remote access, install [Tailscale](https://tailscale.com) on picocluster-claw and your remote devices:
...
Your API keys are stored on picocluster-claw in a JSON file inside the ThreadWeaver container; they never leave the box except when ThreadWeaver makes API calls to that provider.
...
The tool execution always happens on picocluster-claw. The external LLM only sees tool *schemas* (names, descriptions, parameter shapes) and decides when to call them. The flow is:
...
4. ThreadWeaver **executes the tool locally** on picocluster-claw (the LED API gets a POST to port 7777, Blinkt! turns purple).
5. The tool result (`{"status": "ok"}`) goes back to the LLM.
6. The LLM writes its final reply: "Done! The LEDs are now purple."

The external LLM **never touches your hardware directly** — it just describes what it wants, and picocluster-claw acts as the trusted executor. All 28 tools work with every provider: LEDs, temperatures, files, models, time, everything.
...
**If you need strict privacy**, stick with the `local` provider. Everything stays on picocluster-claw/picocrush. The only network traffic is between picocluster-claw and picocrush on the LAN (Ollama API), plus whatever your browser needs to render the UI.
...
- **Files (5 tools)** — sandboxed read/write/list/delete in `/tmp/picocluster-claw-sandbox`
...
### 14. Can the LLM read files on my computer or picocluster-claw's filesystem?

**No.** The `files` MCP server is sandboxed to `/tmp/picocluster-claw-sandbox` inside the ThreadWeaver container. Python's `Path.resolve()` + `relative_to()` checks reject any path that escapes the sandbox, so attempts like `../../../etc/passwd` fail loudly.
...
Your computer's filesystem is never exposed to the LLM. The LLM talks to tools on picocluster-claw; picocluster-claw talks to its own sandbox. There's no path from the LLM to your laptop's files unless you explicitly paste the contents into the chat.

---

### 15. Can I add my own MCP tools?

Yes. See the **Adding Your Own MCP Server** section of [mcp-tools.md](mcp-tools.md) — there's a ~20-line Python template. You drop a new file in `/opt/picocluster-claw/mcp/`, add one line to `threadweaver/start.sh` to auto-connect it, and rebuild the ThreadWeaver container:

```bash
cd /opt/picocluster-claw
...
**From the control panel** (easiest) — open `http://picocluster-claw/` and click "Restart Cluster" or "Shutdown Cluster" in the Cluster Control section.

**From the terminal** on picocluster-claw:

```bash
ssh picocluster@picocluster-claw
pc-restart         # restart both nodes
pc-shutdown        # shut down both nodes
pc-restart picocluster-claw    # restart just this node
...
```bash
curl -X POST http://picocluster-claw:8888/restart \
...
pc-update                 # update everything (picocluster-claw containers + portal + picocrush models)
```
...
The Blinkt! strip on picocluster-claw shows cluster status at a glance:
...
The LED API is reachable at `http://picocluster-claw:7777/` — you can drive it from your own scripts or the control panel's LED buttons. The `pc-led` command wraps it for easy use from the terminal.
...
The control panel (`http://picocluster-claw/`) also pulls some of this data live.
...
```bash
sudo systemctl status picocluster-claw-leds shutdown-api
sudo systemctl restart picocluster-claw-leds
```
...
**Nuclear option** — full cluster restart from picocluster-claw:
```bash
pc-restart all
```
...
Both run simultaneously on picocluster-claw, both talk to the same Ollama backend on picocrush, and both can use the same MCP tools.
...
- **Tailscale** (recommended) — zero-config WireGuard VPN. Install on picocluster-claw and your remote devices, connect through Tailscale IPs instead of LAN IPs.
- **Cloudflare Tunnel** — if you specifically want a public hostname and are willing to authenticate via Cloudflare Access.
- **SSH from a jump host** — forward picocluster-claw's SSH through a public-facing bastion.

Do **not**:

- Port-forward picocluster-claw's SSH directly to the public internet without fail2ban and a rate-limited public key setup.
...
- `sudo docker compose logs -f` in `/opt/picocluster-claw/` on picocluster-claw — live container logs

Or open an issue at [github.com/picocluster/PicoCluster-Claw](https://github.com/picocluster/PicoCluster-Claw).
