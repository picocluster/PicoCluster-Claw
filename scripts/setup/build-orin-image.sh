...
${PARTNER_IP}  clusterclaw clusterclaw.local claw claw.local threadweaver.local control.local
${MY_IP}  clustercrush clustercrush.local crush crush.local
# END PICOCLUSTER CLAW
EOF
  log "Updated /etc/hosts with full cluster aliases"

  # Disable IPv6 — reduces attack surface and avoids SLAAC address churn
